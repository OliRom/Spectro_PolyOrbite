...
void ADCSetup(int n){  // Utiliser le nom de la pin Pmn
  ...
  volatile uint32_t *PmnPFS_ptr = (volatile uint32_t *)(0x40080800 + 0x4 * n);  // Adresse du registre PmnPFS
...
void ADCSelect(int n, bool activation){  // Utiliser le nom de la pin ANmn
  uint16_t mask = pow(2, n);
