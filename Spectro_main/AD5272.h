...
struct _AD5272_cmd_format {
  ...
  _AD5272_cmd_format()
    : data(0), cmd(0){};
  _AD5272_cmd_format(byte cmd, uint16_t data)
    : cmd(cmd), data(data){};

  byte get_w1() {
    return (cmd << 2) | ((data >> 8) & 0b11);
  }  // Obtenir le premier byte de la commande
  byte get_w2() {
    return data & 0xFF;
  }  // Obtenir le second byte de la commande
  uint16_t get_2w() {
    return (cmd << 10) | data;
  }  // Obtenir les 2 bytes de la commande
};
