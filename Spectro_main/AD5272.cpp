...
void AD5272::set_res(uint16_t value) {
  _send_cmd(RDAC_W, value);
}
uint16_t AD5272::get_res() {
  return _send_cmd_resp(RDAC_R);
}
...
byte AD5272::get_ctrl() {
  return (byte)_send_cmd_resp(CTRL_R);
}


void AD5272::software_reset() {
  _send_cmd(SOFT_RST);
}
void AD5272::software_shutdown(bool shut_down) {
  _send_cmd(SOFT_SHUT_DOWN, (uint16_t)shut_down);
}
